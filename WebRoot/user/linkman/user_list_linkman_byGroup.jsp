...
function send(type) {
		...
		if (size > 0) {
			...
			if(type=='del')
				window.location.href="<%=basePath %>user/linkman!delLinkman.action?linkmanIds="+strId+"&group.id=${group.id}";
			if(type=='move'){
				var value = $("#move option:selected").attr("value");
				if(value != -1)
					window.location.href="<%=basePath %>user/linkman!moveLinkman.action?linkmanIds="+strId+"&group.id="+value+"&oldGroupID="+${group.id};
			}
		} else {
			alert("请选择联系人！");
...
function del(){
	var truthBeTold = window.confirm("你确定要删除分组  \"${group.name}\" 吗？ \n删除分组，分组中的联系人也将会被删除。");
	if (truthBeTold) {
		//调用父页面的跳转方法
		parent.skip('<%=basePath%>user/linkman!delGroup.action?group.id=${group.id}');
	}  
}
</script>
  </head>
  ...
  <div class="main">
  <div class="top">
   <input type="button" title="新建分组" value="新建分组" onclick="javascript:window.location.href='<%=basePath %>user/linkman/user_add_group.jsp'"/>
   <input type="button" title="删除分组" value="删除分组" onclick="del()" />
...
<s:iterator value="group.linkmans" status="status">
		<tr class="tr odd">
