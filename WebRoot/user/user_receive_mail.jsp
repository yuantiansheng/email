...
<div class="main">
       ...
       <div class="title">
      		 <b class="inbox" >
      		 <c:if test="${ft == 1}">收件箱</c:if>
      		 <c:if test="${ft == 2}">草稿箱</c:if>
			 <c:if test="${ft == 3}">已发送</c:if>
             <c:if test="${ft == 4}">已删除</c:if>
             <c:if test="${ft == 5}">垃圾邮件</c:if>
      		 </b>&nbsp;&nbsp;(共&nbsp;<b id="font_14">${pageView.totalrecord}</b>&nbsp;封，其中&nbsp;<a href='<%=basePath %>user/receive!seeMail.action?folderType=${ft}&type=seeUnread&page=${pageView.currentpage}'>未读邮件</a>&nbsp;<b class="red font_14">${pageView.unreadMailTotal}</b>&nbsp; 封)
       </div>
       <!--工具-->
       <div class="tool">
       		<div class="menu">
       		<c:if test="${ft != 4}">
            <input type="button"value="删 除" onclick="deleteMail()" />
            </c:if>
            <input  type="button" value="彻底删除" onclick="deleteMailForver()" />
			<select class="sel" onchange="mark()" id="mark">
				<option value="none" selected>标记为...</option>
				<option value="markRead">&nbsp;&nbsp;已读邮件</option>
				<option value="markUnread">&nbsp;&nbsp;未读邮件</option>
			</select>
  
			<select class="sel" onchange="move()" id="move" >
			...
			<select class=sel onchange="see()" id="see">
				<option value="none" selected>查看...</option>
				<option value="seeAll">&nbsp;&nbsp;全部邮件</option>
				<option value="seeUnread">&nbsp;&nbsp;未读邮件</option>
				<option value="seeRead">&nbsp;&nbsp;已读邮件</option>
			</select>
            </div>
             <div class="page">
             <!-- -------------------------分页开始------------------------------------------>
       		<span class="txt">
       			...
       			<span class="unable"><a href="<%=basePath%>user/receive.action?folderType=${ft}&page=1">首页</a></span>&nbsp;
       			...
       			<span class="unable"><a href="<%=basePath%>user/receive.action?folderType=${ft}&page=${wp}" >${wp}</a></span>&nbsp;</c:if>
       			</c:forEach>
       			<span class="unable"><a href="<%=basePath%>user/receive.action?folderType=${ft}&page=${pageView.totalpage}">末页</a></span>
       			</c:if>
       			<!-- 已读 ,未读邮件分页 -->
       			<c:if test="${(type == 'seeUnread') ||(type == 'seeRead')}">
       			<span class="unable"><a href="<%=basePath%>user/receive!seeMail.action?folderType=${ft}&page=1&type=${type}">首页</a></span>&nbsp;
       			<c:forEach var="wp" begin="${pageView.pageindex.startIndex}" end="${pageView.pageindex.endIndex}">
       			<c:if test="${pageView.currentpage == wp}"><b>${wp}</b></c:if>
       			<c:if test="${pageView.currentpage != wp}">
       			<span class="unable"><a href="<%=basePath%>user/receive!seeMail.action?folderType=${ft}&page=${wp}&type=${type}" >${wp}</a></span>&nbsp;</c:if>
       			</c:forEach>
       			<span class="unable"><a href="<%=basePath%>user/receive!seeMail.action?folderType=${ft}&page=${pageView.totalpage}&type=${type}">末页</a></span>
       			</c:if>
       		</span>
       		<!-- -------------------------分页结束-------------------------------------------->
       </div>
