package cn.edu.pdsu.service.mail.receive;
...
public class ReceiveMail {
	...
	private void connection() throws MessagingException {
		Session session = MailSession
				.getInstance(MailConstant.EMAIL_CONFIG_PATH);
	...
	private MailBean setMailBean(IMAPMessage m) throws Exception {
		...
		mail.setCc(decode(InternetAddress.toString(m
				.getRecipients(Message.RecipientType.CC))));
		mail.setTo(decode(InternetAddress.toString(m
				.getRecipients(Message.RecipientType.TO))));
		mail.setMessageID(m.getMessageID());
		mail.setSize(m.getSize());
		mail.setMessageUID(folder.getUID(m));
		mail.setFlags(setFlags(m));
		mail.setFolderType(UserReceiveMailService.folderType(folderName));
		return mail;
	}
	...
	private void setContent(MailBean mailbean)
		...
		if (filename.length() > 0) {
			mailbean.setFile(filename.deleteCharAt(filename.length() - 1)
					.toString());
		}
	}
